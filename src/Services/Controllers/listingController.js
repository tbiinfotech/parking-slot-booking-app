    ...
    try {
        ...
        if (addresses.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'No addresses found for this owner.',
            });
        }
...
exports.getAddressByUserLocation = async (req, res) => {
    try {
        ...
        const listings = await Listing.find({
            $and: [{ ownerId }, { type: userInfo.preference }],
        })
            .populate('owner')
            .sort({ createdAt: -1 });
        // Extract addresses from listings
...
exports.addToFavorites = async (req, res) => {
    try {
        ...
        if (user.favorites.includes(listingId)) {
            ...
            await user.save();
            return res.status(200).json({ success: true, message: 'Space removed from favourites list successfully.' });
        }
        ...
        res.status(200).json({ success: true, message: 'Space added to favourites list successfully.' });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
};
...
exports.getFavoriteAddresses = async (req, res) => {
    try {
        ...
        if (favoriteAddresses.length === 0) {
            return res
                .status(404)
                .json({ success: false, message: 'No favorite addresses found.' });
        }
    ...
    try {
        ...
        if (listing.status !== 'Available') {
            return res.status(400).json({
                success: false,
                message: 'Listing status is not available to update.',
            });
        }
        ...
        res.status(200).json({
            success: true,
            message: 'Listing status updated to Rent Out.',
            listing,
        });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
...
exports.removeListing = async (req, res) => {
    try {
        ...
        res
            .status(200)
            .json({ success: true, message: 'Space removed successfully.' });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
};
...
exports.editListing = async (req, res) => {
    try {
        ...
        const rentalParkingPlan =
            req.body.rentalParkingPlan || listing.rentalParkingPlan;
        ...
        const location = req.body.location
            ? {
                address:
                    req.body.location.address?.trim() || listing.location.address,
                coordinates: [
        ...
        const dimensions = req.body.dimensions
            ? {
                length:
                    parseFloat(req.body.dimensions.length) || listing.dimensions.length,
                width:
                    parseFloat(req.body.dimensions.width) || listing.dimensions.width,
                height:
                    parseFloat(req.body.dimensions.height) || listing.dimensions.height,
            }
            : listing.dimensions;
        ...
        if (req.files && req.files.length > 0) {
            ...
            listing.photos.forEach((photoPath) => {
                fs.unlink(photoPath, (err) => {
                    if (err)
                        console.error(`Failed to delete old photo at ${photoPath}:`, err);
                });
            });

            // Add new photos
            listing.photos = req.files.map((file) =>
...
exports.updateListing = async (req, res) => {
    ...
    try {
        ...
        if (req.files) {
            ...
            Object.keys(reqFiles).forEach((key, index) => {
                ...
                if (fileArray.length > 0) {
                    const newFile = fileArray[0]; // take the first file in the array
                    let oldPhotoPath;
                    if (index < listing.photos.length && index === listing.photos.findIndex(photo => photo === listing.photos[index])) {
                    ...
                    oldPhotos[number] = newFile.path.replace(/^public\//, '');
                }
            });

            listing.photos = oldPhotos.filter(Boolean) // Filter out any undefined values
        }
        ...
        const updateData = {
            ...
            ...(req.files && { photos: listing.photos }) // Update photos if new ones were uploaded
        };
...
function filterByPlanType(listings, type) {
    const updatedListingByPlan = listings.filter(
        ...
        if (radiusInMiles || type || plan || minPrice || maxPrice) {
            ...
            if (!latitude || !longitude || !radiusInKilometers) {
                return res.status(400).json({
                    success: false,
                    message:
                        'Please provide latitude, longitude, and radiusInKilometers.',
                });
            }

            // Log the coordinates being passed
            console.log('Querying listings around coordinates:', [
                longitude,
                latitude,
            ]);
...
exports.updateListingAvailability = async (req, res) => {
    console.log('#updateListingAvailability');
    try {
        const { id } = req.params; // Get listing ID from URL parameters
        const { status } = req.body; // Get listing ID from URL parameters
        ...
        // } else if ((listing.status).toLowerCase() === 'unavailable') {
        //     listing.status = 'Available';
        // } else if ((listing.status).toLowerCase() === 'rent out') {
        //     listing.status = 'Rent Out';
        // } else {
        //     listing.status = 'Unavailable';
        // }
